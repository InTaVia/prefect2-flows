...
@task(tags=["render-rdflib"], timeout_seconds=60)
def render_personperson_relation(rel):
...
def render_organizationplace_relation(rel):
    ...
    if (
        ...
        "related_place"
    ]["id"] not in glob_list_entities["place"]:
        place = rel["related_place"]["id"]
...
def gather_render_tasks(data, render_task):
    res = []
    for item in data:
        res.extend(render_task.map(item))
    return res
...
def create_apis_rdf_serialization_v3(params: Params):
    ...
    pers_counts = request_from_api(
        filter_params=params.filter_params, return_count_only=True, entity_type="person"
    )
